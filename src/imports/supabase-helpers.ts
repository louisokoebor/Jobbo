...
// User-context client — anon key + caller's JWT.
// RLS is enforced. Use for all user-data reads/writes.
function sbUser(jwt: string) {
  return createClient(
    Deno.env.get("SUPABASE_URL")!,
...
async function requireAuth(c: any, next: any) {
  ...
  // Reject bare anon key being passed as user token
  if (jwt === Deno.env.get("SUPABASE_ANON_KEY")) {
    return c.json({ success: false, error: "unauthorized" }, 401);
  }

  const supabase = sbUser(jwt);

  // Server-side validation — network-verified, detects revoked sessions
  const { data: { user }, error } = await supabase.auth.getUser(jwt);
  if (error || !user) {
    console.log("[requireAuth] invalid JWT:", error?.message ?? "no user");
    return c.json({ success: false, error: "unauthorized" }, 401);
  }
...
// Apply auth middleware to all routes except health and stripe-webhook
app.use("/make-server-3bbff5cf/*", async (c, next) => {
  const path = c.req.path;
  if (
    path.endsWith("/make-server-3bbff5cf/health") ||
    path.endsWith("/make-server-3bbff5cf/stripe-webhook")
  ) {
    return next();
  }
...
app.post("/make-server-3bbff5cf/create-checkout-session", async (c) => {
  ...
  try {
    ...
    if (!customerId) {
      ...
      // Use admin to write stripe_customer_id (entitlement field)
      await sbAdmin().from("users").update({ stripe_customer_id: customerId }).eq("id", userId);
    }
...
// ── Stripe Webhook ────────────────────────────────────────────────────────────
// No user JWT — verified by Stripe signature instead. Uses sbAdmin().
app.post("/make-server-3bbff5cf/stripe-webhook", async (c) => {
  ...
  if (
    ...
    const { data: user } = await admin.from("users").select("id").eq("stripe_customer_id", customerId).single();
...
app.post("/make-server-3bbff5cf/generate-cv", async (c) => {
  ...
  // Use admin for entitlement check — must not be bypassable
  const admin = sbAdmin();
  ...
  if (extractedTerms && (extractedTerms.mustHaves?.length > 0 || extractedTerms.skills?.length > 0)) {
    ...
    const missingKeywords = skillTerms.filter((s: string) => !cvText.includes(s.toLowerCase())).slice(0, 15);
...
- Never fabricate experience`;
    ...
    const missingKeywords = [...allJobSkills].slice(0, 15);
...
The skills_gap field should list skills from the job the candidate genuinely lacks — be honest, do not pad this list.`;
  ...
  try {
    ...
    yearsLabel = actualYears >= 10 ? 'over 10 years' : actualYears > 0 ? `${actualYears}+ years` : null;
...
IMPORTANT: When writing the summary or any bullet that references years of experience, always use the candidate's ACTUAL years above. Never use the years requirement from the job description as the candidate's experience level.
` : '';
...
CORE PRINCIPLES:
1. Mirror the employer's language. Use the same terminology the job description uses.
2. Evidence over assertion. Never write 'strong communication skills'. Show, don't tell.
3. Strong evidence placement. Requirements from the job description must appear in experience bullets.
4. Quantify everything possible. Keep and amplify any numbers, dates, team sizes, budget values.
5. Verb precision. Start every bullet with a strong, specific verb matching the seniority level.
6. Honest tailoring only. Only include skills and experience the candidate actually has.
7. Summary is a pitch. Reference the job title or field, the candidate's most relevant strength, and a forward-looking statement. Maximum 4 sentences.
8. Never mirror the job description's experience requirement back as the candidate's experience level.
...
YOUR TASK:
Rewrite this CV to be the strongest possible application for the role above.
...
2. EXPERIENCE BULLETS — rules per role:
   ...
   - If fewer than 4 bullets exist, expand by breaking compound bullets or adding context
   - Never produce a role with 1, 2, or 3 bullets under any circumstance
   - Start each bullet with a strong past or present tense verb
   - Keep any metrics/numbers from the original
...
3. SKILLS — reorder most relevant first. 8-12 items maximum.

4. SKILLS GAP — genuine gaps only. Empty array if none.
...
Do not include any explanation, preamble, or markdown. JSON only.`;
  ...
  try {
    ...
    try {
      if (generatedCv.work_history && Array.isArray(generatedCv.work_history)) {
        generatedCv.work_history = generatedCv.work_history.map((role: any) => {
          ...
          if (bullets.length < 4) {
            console.log(`[generate-cv] role "${role.title} at ${role.company}" only has ${bullets.length} bullets — padding to 4`);
            const originalRole = (cvData.work_history || []).find((r: any) => r.company?.toLowerCase() === role.company?.toLowerCase());
            ...
            for (const originalBullet of originalBullets) {
              ...
              const alreadyIncluded = mergedBullets.some((b: string) => b.toLowerCase().slice(0, 30) === originalBullet.toLowerCase().slice(0, 30));
              if (!alreadyIncluded) mergedBullets.push(originalBullet);
            }
    ...
    if (saveErr) return c.json({ success: false, error: "Failed to save: " + saveErr.message }, 500);

    // Increment generations_used (entitlement — use admin)
    const { error: incrementError } = await admin
      .from("users")
      .update({ generations_used: generationsUsed + 1, updated_at: new Date().toISOString() })
      .eq("id", userId);
...
app.get("/make-server-3bbff5cf/generated-cv/:id", async (c) => {
  ...
  try {
    ...
    return c.json({ success: true, cv_json: g.cv_json, template: g.template_id || "clean", application_id: g.application_id, job_title: appData?.job_title || "", company: appData?.company || "", job_description_raw: appData?.job_description_raw || "" });
...
app.post("/make-server-3bbff5cf/generate-cover-letter", async (c) => {
  ...
  // Plan gate — use admin for entitlement check
...
Return only letter body text (4 paragraphs, no headers/signature).`;
    ...
    if (!r.ok) return c.json({ success: false, error: "AI generation failed." }, 500);
    ...
    // Increment generations_used (admin — entitlement update)
    try {
      ...
      await admin.from("users").update({ generations_used: clUsed + 1, updated_at: new Date().toISOString() }).eq("id", userId);
    } catch (incErr) {
...
app.post("/make-server-3bbff5cf/improve-bullet", async (c) => {
  ...
  if (!bulletText) return c.json({ success: false, error: "bulletText required" }, 400);
...
Return only the improved bullet text.`;
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      method: "POST",
      headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" },
      body: JSON.stringify({ model: "gpt-4o-mini", temperature: 0.3, max_tokens: 150, messages: [{ role: "system", content: systemPrompt }, { role: "user", content: userPrompt }] }),
    });

    if (!response.ok) {
      console.log("[improve-bullet] OpenAI error:", response.status, await response.text());
      return c.json({ success: false, error: "AI request failed" }, 500);
    }

    const data = await response.json();
    const improved = data.choices?.[0]?.message?.content?.trim() ?? "";
    if (!improved || improved.length < 10) return c.json({ success: false, error: "No improvement generated" }, 500);
...
function pdfDrawWorkExperience(ctx: PDFCtx, workHistory: any[]) {
  for (const role of workHistory) { pdfCheckPageBreak(ctx, 40);
    ...
    const bullets = Array.isArray(role.bullets) ? role.bullets : [];
    for (const b of bullets) { pdfCheckPageBreak(ctx, 16); const bt = typeof b === "string" ? b : (b.text ?? ""); if (bt) pdfDrawText(ctx, `\u2014 ${bt}`, { font: ctx.regularFont, size: 9.5, color: PDF_DARK_GREY, indent: 8, lineHeight: 9.5 * 1.4 }); }
    ctx.y -= 8;
  }
}
...
app.post("/make-server-3bbff5cf/generate-pdf", async (c) => {
  // userId verified by middleware, no need to use it for PDF generation
  let cvJson: any;
  try {
    const b = await c.req.json();
    cvJson = b.cv_json;
...
app.post("/make-server-3bbff5cf/generate-interview-prep", async (c) => {
  ...
  let applicationId: string, loadOnly = false, forceRegenerate = false;
  try {
    const b = await c.req.json();
  ...
  if (existing?.questions?.length > 0 && !forceRegenerate) {
    const questions = isPro ? existing.questions : existing.questions.slice(0, 5);
  ...
  if (!generatedCv) return c.json({ error: "Please generate a CV for this application first", code: "NO_GENERATED_CV" }, 400);
...
- Mix question types: technical, behavioural, experience, motivation, competency
- Suggested answers MUST draw from the candidate's ACTUAL experience in their CV
- For behavioural questions use STAR format
...
Return this exact JSON schema:
{
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      method: "POST",
      headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" },
      body: JSON.stringify({ model: "gpt-4o", temperature: 0.5, max_tokens: 4000, response_format: { type: "json_object" }, messages: [{ role: "system", content: sysPrompt }, { role: "user", content: userPrompt }] }),
    });

    if (!response.ok) {
      console.log("[interview-prep] OpenAI error:", await response.text());
...
// ── Interview Prep — save practice answer ────────────────────────────────────
...
// ── Delete Account ─────────────────────────────────────────────────────────────
// Legitimate service-role use: cascading deletes + auth.admin.deleteUser
app.delete("/make-server-3bbff5cf/delete-account", async (c) => {
  ...
  try { await admin.from("interview_prep").delete().eq("user_id", userId); } catch (e) { console.log("[delete-account] interview_prep (continuing):", e); }
  try { await admin.from("cover_letters").delete().in("application_id", admin.from("applications").select("id").eq("user_id", userId)); } catch (e) { console.log("[delete-account] cover_letters (continuing):", e); }
  try { await admin.from("generated_cvs").delete().in("application_id", admin.from("applications").select("id").eq("user_id", userId)); } catch (e) { console.log("[delete-account] generated_cvs (continuing):", e); }
  try { await admin.from("applications").delete().eq("user_id", userId); } catch (e) { console.log("[delete-account] applications (continuing):", e); }
  try { await admin.from("cv_profiles").delete().eq("user_id", userId); } catch (e) { console.log("[delete-account] cv_profiles (continuing):", e); }
  try { await admin.from("users").delete().eq("id", userId); } catch (e) { console.log("[delete-account] users (continuing):", e); }

  try {
    const { data: files } = await admin.storage.from("cv-uploads").list(userId);
    if (files && files.length > 0) {
      await admin.storage.from("cv-uploads").remove(files.map((f: any) => `${userId}/${f.name}`));
    }
  } catch (e) { console.log("[delete-account] storage (continuing):", e); }

  try {
    await admin.auth.admin.deleteUser(userId);
  } catch (e) { console.log("[delete-account] auth user (continuing):", e); }
...
app.post('/make-server-3bbff5cf/patch-cv-gap', async (c) => {
  ...
  let applicationId: string, generatedCvId: string, gapTerm: string, gapType: string, userContext: string;
  try {
    const b = await c.req.json();
    applicationId = b.application_id;
...
If gap_type is "experience": find the most relevant role, either UPDATE an existing bullet (preferred) or ADD a new one.
...
Return JSON: { "patch_type": "add_skill"|"update_bullet"|"add_bullet"|"add_certification", "target_role_index": 0, "bullet_index": 2, "new_skill": "...", "new_bullet": "...", "new_certification": "...", "explanation": "..." }`;

  try {
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      method: 'POST',
      headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: 'gpt-4o-mini', temperature: 0.2, max_tokens: 400, response_format: { type: 'json_object' }, messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }] }),
    });
